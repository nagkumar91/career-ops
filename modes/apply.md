...
4. LOAD       → Read complete report + Section G (if it exists)
5. COMPARE    → Does the role on screen match the evaluated one? If it changed → alert
...
5. If there's NO match → alert and offer to run a quick auto-pipeline
...
- **If re-evaluate**: Run the complete A-F evaluation, update the report, regenerate Section G
