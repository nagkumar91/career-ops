...
2. **web_fetch (fallback):** For static pages (ZipRecruiter, WeLoveProduct, company career pages).
3. **web_search (last resort):** Search for the role title + company on secondary portals that index the JD in static HTML.
...
Run exactly the same as the `oferta` mode (read `modes/oferta.md` for all blocks A-F).
