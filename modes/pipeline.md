...
2. **For each pending URL**:
   ...
   b. **Extract JD** using Playwright (browser_navigate + browser_snapshot) → web_fetch → web_search
...
2. **web_fetch (fallback):** For static pages or when Playwright is not available.
3. **web_search (last resort):** Search on secondary portals that index the JD.
